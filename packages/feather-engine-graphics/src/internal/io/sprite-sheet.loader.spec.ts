...
describe('SpriteSheetLoader', () => {
  ...
  it('should load a spritesheet with frames', async () => {
  ...
  it('should load a spritesheet with anim', async () => {
    spriteSheetLoader['loadSheetSpec'] = async () =>
      new Promise(e => e({ animations: [{ name: 'a', frames: ['1', '2', '3'], frameLen: 15 }] } as SpriteSheetModel));
    const sprite = await spriteSheetLoader.load();
    expect(sprite).toBe(spriteSheet);
