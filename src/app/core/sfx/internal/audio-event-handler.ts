...
export const playSfxOn: (audioBord: AudioBoard) => Receiver = audioBoard => ({
...
export const playBgmOn: (audioBord: AudioBoard) => Receiver = audioBoard => ({
  receive: (sfxEvent: Events.PlayBgmEvent): void => {
    const { name } = sfxEvent.payload;
    audioBoard.playBgm(name);
  },
});
export const setBgmVolumeOn: (audioBord: AudioBoard) => Receiver = audioBoard => ({
  receive: (sfxEvent: Events.SetBgmVolumeEvent): void => {
    const { value } = sfxEvent.payload;
    audioBoard.setBgmVolume(value);
  },
});
export const setSfxVolumeOn: (audioBord: AudioBoard) => Receiver = audioBoard => ({
  receive: (sfxEvent: Events.SetSfxVolumeEvent): void => {
    const { value } = sfxEvent.payload;
    audioBoard.setSfxVolume(value);
  },
});
export const setMasterVolumeOn: (audioBord: AudioBoard) => Receiver = audioBoard => ({
